-- Update calculate_commission_on_completion to read platform rate from settings
...
-- Fix existing commission records to use the correct platform rate (10%)
UPDATE commission_ledger
SET 
  platform_rate = 10,
  platform_commission = gross_margin * 0.10,
  corner_commission = CASE WHEN corner_id IS NOT NULL THEN gross_margin * 0.10 ELSE 0 END,
  centro_commission = gross_margin * 0.70,
  updated_at = now()
WHERE platform_rate = 20;