...
-- Add field to track prepaid commission on quotes
...
-- Create function to charge commission on quote acceptance
...
BEGIN
  ...
  IF NEW.status = 'accepted' AND NEW.signed_at IS NOT NULL 
    ...
    IF NEW.repair_request_id IS NOT NULL THEN
      SELECT corner_id, assigned_provider_id, assigned_provider_type 
...
DROP TRIGGER IF EXISTS trigger_charge_commission_on_quote_acceptance ON quotes;
CREATE TRIGGER trigger_charge_commission_on_quote_acceptance
  BEFORE UPDATE ON quotes
  FOR EACH ROW
  EXECUTE FUNCTION charge_commission_on_quote_acceptance();

-- Also handle INSERT case for quotes created already accepted
DROP TRIGGER IF EXISTS trigger_charge_commission_on_quote_insert ON quotes;
CREATE TRIGGER trigger_charge_commission_on_quote_insert
