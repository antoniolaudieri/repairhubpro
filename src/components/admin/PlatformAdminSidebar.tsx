    ...
    <Sidebar collapsible="icon" className="border-r border-sidebar-border">
      <div className="h-full bg-sidebar text-sidebar-foreground flex flex-col">
        <SidebarHeader className="h-16 border-b border-sidebar-border flex items-center justify-center">
          ...
          >
            <motion.div 
              className="h-9 w-9 rounded-xl overflow-hidden flex items-center justify-center bg-gradient-to-br from-primary to-primary/80 shadow-lg shadow-primary/25"
            ...
            <AnimatePresence mode="wait">
              {!isCollapsed && (
                ...
                >
                  <span className="text-sm font-bold text-sidebar-foreground">Admin</span>
                  <span className="text-xs text-sidebar-foreground/60">Piattaforma</span>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
            ...
            <SidebarGroupContent>
              <SidebarMenu className="space-y-1">
                {menuItems.map((item, index) => {
                  ...
                  const pendingCount = item.countKey && pendingCounts ? pendingCounts[item.countKey] : 0;
                      ...
                      <SidebarMenuItem>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <Icon className={`h-5 w-5 flex-shrink-0 ${pendingCount > 0 ? 'text-warning' : ''}`} />
                            </motion.div>
                            <AnimatePresence mode="wait">
                              {!isCollapsed && (
                                <motion.div 
                                ...
                                >
                                  ...
                                  {pendingCount > 0 && (
                                    <Badge className="h-5 min-w-[20px] px-1.5 bg-warning text-warning-foreground text-xs animate-pulse">
                                      {pendingCount}
                                    </Badge>
                                  )}
                                </motion.div>
                            ...
                            {isCollapsed && pendingCount > 0 && (
                              <Badge className="absolute -top-1 -right-1 h-4 min-w-[16px] px-1 bg-warning text-warning-foreground text-[10px] animate-pulse">
                                {pendingCount}
                              </Badge>
                            )}
                          </NavLink>
